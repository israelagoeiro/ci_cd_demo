# Script para criar issues no GitHub de forma interativa
# Este script solicita o token de acesso pessoal do GitHub durante a execução
...
$token = Read-Host "Digite seu token de acesso pessoal do GitHub (o texto não será exibido)" -AsSecureString
...
if ([string]::IsNullOrEmpty($token)) {
    Write-Host "Token não fornecido. Encerrando o script." -ForegroundColor Red
    exit 1
}
