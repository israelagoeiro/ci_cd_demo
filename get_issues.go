...
// Função para filtrar issues por label
func filterIssuesByLabel(issues []Issue, labelName string) []Issue {
	if labelName == "" {
		return issues
	}
...
// Função para filtrar issues por estado (open/closed)
...
func main() {
	...
	// Verificar argumentos da linha de comando para filtros
	...
	for i := 0; i < len(args); i++ {
		if args[i] == "--label" && i+1 < len(args) {
			...
			i++
		} else if args[i] == "--state" && i+1 < len(args) {
	...
	if stateFilter != "" {
		filteredIssues = filterIssuesByState(filteredIssues, stateFilter)
	}

	if labelFilter != "" {
		filteredIssues = filterIssuesByLabel(filteredIssues, labelFilter)
	}

	// Construir mensagem de status
	statusMsg := fmt.Sprintf("Encontradas %d issues", len(filteredIssues))
	if labelFilter != "" {
		statusMsg += fmt.Sprintf(" com a label '%s'", labelFilter)
	}
	if stateFilter != "" {
		statusMsg += fmt.Sprintf(" no estado '%s'", stateFilter)
	}
	statusMsg += fmt.Sprintf(" no repositório %s/%s:\n\n", owner, repo)
	...
	jsonData, err := json.MarshalIndent(filteredIssues, "", "  ")
	if err != nil {
